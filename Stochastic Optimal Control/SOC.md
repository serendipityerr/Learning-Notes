...
### Girsanov Theorem and Measure Transformation

In this section, we consider the transformation of probability measures for continuous-time stochastic processes. This theorem provides the mathematical foundation for connecting uncontrolled random dynamics with controlled dynamics, which is crucial for Path Integral Control.

Consider a base stochastic dynamics (uncontrolled process) defined by a Stochastic Differential Equation (SDE) under the probability measure $\mathbb{P}$:

$$\mathrm{d}x_t = f(x_t, t) \mathrm{d}t + g_t \mathrm{d}w_t^{\mathbb{P}}$$

where $w_t^{\mathbb{P}}$ is the standard Wiener process under measure $\mathbb{P}$.

Now, assume we apply an external control $u_t$ to the system, altering its drift. The controlled dynamics under a new target measure $\mathbb{Q}$ is given by:

$$\mathrm{d}x_t = \left( f(x_t, t) + g_t u_t \right) \mathrm{d}t + g_t \mathrm{d}w_t^{\mathbb{Q}}$$

where $w_t^{\mathbb{Q}}$ is the standard Wiener process under the new measure $\mathbb{Q}$. Notice that the diffusion coefficient $g_t$ must remain identical for the two measures to be equivalent (mutually absolutely continuous).

Girsanov Theorem states that the Radon-Nikodym derivative (the probability density ratio of the path measures) between the controlled universe $\mathbb{Q}$ and the uncontrolled universe $\mathbb{P}$ over the time interval $[0, T]$ is given by:

$$\frac{\mathrm{d}\mathbb{Q}}{\mathrm{d}\mathbb{P}} = \exp\left( \int_0^T u_t^\top \mathrm{d}w_t^{\mathbb{P}} - \frac{1}{2} \int_0^T \|u_t\|^2 \mathrm{d}t \right)$$

Since the path distributions are different, we are often interested in the information-theoretic distance between them, namely the Kullback-Leibler (KL) divergence. We evaluate the expectation of the log-likelihood ratio under the controlled measure $\mathbb{Q}$:

$$D_{KL}(\mathbb{Q} \parallel \mathbb{P}) = \mathbb{E}_{\mathbb{Q}} \left[ \log \frac{\mathrm{d}\mathbb{Q}}{\mathrm{d}\mathbb{P}} \right]$$

To compute this, we first substitute the relationship between the Wiener processes under the two measures, $\mathrm{d}w_t^{\mathbb{P}} = \mathrm{d}w_t^{\mathbb{Q}} + u_t \mathrm{d}t$, into the integral:

$$
\begin{aligned}
\mathbb{E}_{\mathbb{Q}} \left[ \log \frac{\mathrm{d}\mathbb{Q}}{\mathrm{d}\mathbb{P}} \right] &= \mathbb{E}_{\mathbb{Q}} \left[ \int_0^T u_t^\top \left( \mathrm{d}w_t^{\mathbb{Q}} + u_t \mathrm{d}t \right) - \frac{1}{2} \int_0^T \|u_t\|^2 \mathrm{d}t \right] \\
&= \mathbb{E}_{\mathbb{Q}} \left[ \int_0^T u_t^\top \mathrm{d}w_t^{\mathbb{Q}} + \int_0^T \|u_t\|^2 \mathrm{d}t - \frac{1}{2} \int_0^T \|u_t\|^2 \mathrm{d}t \right] \\
&= \mathbb{E}_{\mathbb{Q}} \left[ \int_0^T u_t^\top \mathrm{d}w_t^{\mathbb{Q}} \right] + \mathbb{E}_{\mathbb{Q}} \left[ \int_0^T \frac{1}{2} \|u_t\|^2 \mathrm{d}t \right]
\end{aligned}
$$

A fundamental property of the Itô integral is that the integral with respect to Brownian motion $\int u_t^\top \mathrm{d}w_t^{\mathbb{Q}}$ is a martingale under the measure $\mathbb{Q}$, meaning its expectation evaluates to zero:

$$\mathbb{E}_{\mathbb{Q}} \left[ \int_0^T u_t^\top \mathrm{d}w_t^{\mathbb{Q}} \right] = 0$$

Therefore, the equation elegantly simplifies to:

$$D_{KL}(\mathbb{Q} \parallel \mathbb{P}) = \mathbb{E}_{\mathbb{Q}} \left[ \int_0^T \frac{1}{2} \|u_t\|^2 \mathrm{d}t \right]$$

We can easily find that the KL divergence (information-theoretic distance) between the controlled and uncontrolled path measures is exactly equivalent to the cumulative quadratic control cost. This perfectly explains why minimizing the control effort in Stochastic Optimal Control is mathematically equivalent to minimizing the discrepancy between probability distributions.