...
**Kullback-Leibler (KL) Divergence** (also known as **Relative Entropy**) is a metric used to measure the difference between two probability distributions.
...
As for two continuous probability distributions $p$ and $q$, the KL Divergence is defined as: 

$$
D_{\text{KL}}(p \ \Vert \ q) = \int_{\mathcal{X}} p(x) \log \frac{p(x)}{q(x)} \mathrm{d} x = \mathbb{E}_{x \sim p(x)} \left[ \log \frac{p(x)}{q(x)} \right].
$$
...
1. **Non-negativity (Gibbs' Inequality)**: $D_{\text{KL}}(p \ \Vert \ q) \geq 0$ and the equality holds iff $p(x) = q(x)$ (almost anywhere). 
2. **Asymmetry**: Generally, $D_{\text{KL}}(p \ \Vert \ q) \neq D_{\text{KL}}(q \ \Vert \ p)$. Therefore, the KL Divergence is **not** a true distance metric.
...
1. Forward KL Divergence: $\min_{q_ \theta} D_{\text{KL}}(p \ \Vert \ q_ \theta)$
2. Reverse KL Divergence: $\min_{q_ \theta} D_{\text{KL}}(q_ \theta \ \Vert \ p)$

### Ideal Cases
In ideal (unrestricted) scenario, we have

$$
\begin{gathered}
\min_{q_ \theta} D_{\text{KL}}(p \ \Vert \ q_ \theta) = 0 \ \Rightarrow \ q_ \theta^*(x) = p(x), \\
\min_{q_ \theta} D_{\text{KL}}(q_ \theta \ \Vert \ p) = 0 \ \Rightarrow \ q_ \theta^*(x) = p(x),
\end{gathered}
$$

which means that the optimal solution is the target distribution itself. Therefore, in this case, optimizing both Forward and Reverse KL Divergence are equivalent.
...
In this case, the objective functions for the Forward and Reverse KL Divergence will cause the parameters $\theta^*$ to **converge in completely different directions**. We will perform a rigorous derivation using the example of fitting an arbitrary complex distribution with a single Gaussian distribution.
...
$$
\min_ \theta D_{\text{KL}}(p \ \Vert \ q_\theta) = \min_ \theta \mathbb{E}_{x \sim p(x)} \left[\log \frac{p(x)}{q_\theta(x)}\right] = \min_ \theta \mathbb{E}_{x \sim p(x)} [\log p(x) - \log q_\theta(x)],
$$

where minimizing $q_ \theta$ is equivalent to minimizing $\theta$ because $q_ \theta$ is completely determined by the parameter $\theta$. Since $\mathbb{E}_{x \sim p(x)}[\log p(x)]$ is constant w.r.t $\theta$, this is equivalent to **Maximum Likelihood Estimation (MLE)**:
...
$$
\begin{aligned}
\min_ \theta D_{\text{KL}}(q_\theta \ \Vert \ p) &= \min_ \theta \mathbb{E}_{x \sim q_\theta(x)} \left[\log \frac{q_\theta(x)}{p(x)}\right] \\
&= \min_ \theta \left[ -H(q_\theta(x)) + \mathbb{E}_{x \sim q_\theta(x)} \left[ -\log p(x) \right] \right].
\end{aligned}
$$

The objective function here consists of two parts, which is the essence of maximizing the ELBO in variational inference:
1. **The cross-entropy term** $\mathbb{E}_ {q_ \theta}[-\log p(x)]$ requires that $q_\theta(x)$ be concentrated in regions where $p(x)$ is large. If $q_\theta$ allocates probability mass to the “valley” regions where $p(x) \approx 0$, causing $-\log p(x) \to \infty$, the penalty from this term will instantly explode.
2. **Negative entropy term** $-H(q_ \theta)$ requires that the variance of $q_\theta(x)$ itself be as small as possible (the more concentrated the distribution, the smaller the negative entropy).
